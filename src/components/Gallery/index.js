...
class Gallery extends Component {
  ...
  getImageDetails = () => {
    ...
    const imageDetails = imagesList.filter(
      eachImageDetails => eachImageDetails.id === activeImageId,
    )
    return imageDetails
  }
  ...
  render() {
    ...
    const {activeImageId} = this.state
    const {imageUrl, imageAltText} = imageDetails[0]
        ...
        <ul className="list-container">
          {imagesList.map(eachImageDetails => (
            <ThumbnailItem
              ...
              updateThumbnailActiveId={this.updateThumbnailActiveId}
              isActive={eachImageDetails.id === activeImageId}
            />
          ))}
        </ul>
      </div>
